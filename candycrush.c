...
void credits()
{
	...
	button=gtk_button_new_with_label("Candy Crush using C");
	gtk_table_attach_defaults(GTK_TABLE(table),button,1,10,1,2);
	gtk_widget_show(button);
	
	button=gtk_button_new_with_label("Mini Project Lab");
	gtk_table_attach_defaults(GTK_TABLE(table),button,1,10,2,3);
	gtk_widget_show(button);
	
	button=gtk_button_new_with_label("BY\nS.HEPHZIBAH(160116733002)\nA.S.K.ANUDEEP(160116733050)");
	gtk_table_attach_defaults(GTK_TABLE(table),button,1,10,3,4);
	gtk_widget_show(button);
	
	button=gtk_button_new_with_label("B.E-2/4  CSE-1");
	gtk_table_attach_defaults(GTK_TABLE(table),button,1,10,4,5);
	gtk_widget_show(button);

	button=gtk_button_new_with_label("CHAITANYA BHARATHI INSTUTUTE OF TECHNOLOGY");
	gtk_table_attach_defaults(GTK_TABLE(table),button,1,10,5,6);
	gtk_widget_show(button);
	
	button=gtk_button_new_with_label("Thank You");
	gtk_table_attach_defaults(GTK_TABLE(table),button,1,10,6,7);
	gtk_widget_show(button);
